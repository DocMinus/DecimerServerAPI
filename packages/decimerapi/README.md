...
`call_image2smiles(...)` keeps backwards-compatible behavior and returns only the `smiles` value.

If you want the full server payload (including metadata like classifier score/decision and reason code), use:

```python
from decimerapi.decimerapi import DecimerAPI

decimer_api = DecimerAPI()
result = decimer_api.call_image2smiles_with_meta(input_image)
print(result)
```
